...
class RegularBehavior(CommitBehavior):
    ...
    def _generate_time_off(self, start_date: datetime, end_date: datetime):
        ...
        while days_allocated < vacation_days:
            ...
            vacation_start = start_date + timedelta(days=random.randint(0, total_days - 14))
            # Adjust to Monday
            ...
            vacation_length = random.randint(min_vacation, max_vacation) if min_vacation <= max_vacation else min_vacation
        ...
        for _ in range(sick_days):
            ...
            for i in range(sick_length):
                ...
                if start_date <= sick_day <= end_date and not self._is_weekend(sick_day):
...
class IntenseBehavior(CommitBehavior):
    ...
    def get_commits_for_day(self, date: datetime, context: Dict) -> int:
        ...
        elif cycle_day < 8:  # Recovery (days 5-7)
            return random.choices([0, 1, 2, 3], weights=[50, 30, 15, 5])[0]
            
        else:  # Normal pace (days 8-11)
...
class HobbyistBehavior(CommitBehavior):
    ...
    def get_commits_for_day(self, date: datetime, context: Dict) -> int:
        ...
        if self._is_weekend(date):
            ...
            if random.random() < (0.3 if is_winter else 0.2):
...
def get_behavior(behavior_type: str, max_commits: int = 10, frequency: int = 80) -> CommitBehavior:
