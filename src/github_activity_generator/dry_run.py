...
class DryRunSimulator:
    ...
    def simulate_commit(
        ...
        if self.verbose:
            self.console.print(
                f"[green]WOULD CREATE:[/green] Commit on {date.strftime('%Y-%m-%d %H:%M')} "
                f"- {message}"
            )
...
def display_dry_run_report(report: DryRunReport, console: Optional[Console] = None) -> None:
    ...
    if report.commits_by_day:
        ...
        for day in sample_days:
            ...
            for i, commit in enumerate(commits[:3]):  # Show max 3 commits per day
                console.print(
                    f"    {i+1}. {commit.date.strftime('%H:%M')} - {commit.message}"
                )
            if len(commits) > 3:
                console.print(f"    ... and {len(commits) - 3} more commits")
...
def save_dry_run_report(
    ...
    Args:
        ...
        format: Format to save in ("text", "json", "csv")
    """
    if format == "json":
        import json
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["Date", "Time", "Day of Week", "Message", "Is Weekend", "Is Holiday"])
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(f"Date Range: {report.start_date.strftime('%Y-%m-%d')} to "
                   f"{report.end_date.strftime('%Y-%m-%d')}\n\n")
