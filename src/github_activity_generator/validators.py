...
from .constants import MAX_COMMITS_PER_DAY, MAX_FREQUENCY, MIN_URL_PARTS
...
def validate_date_string(date_str: str) -> str:
    ...
    if not isinstance(date_str, str):
        error_msg = f"Date must be a string, got {type(date_str)}"
    ...
    if date_str.lower().endswith("_days_ago"):
        try:
            ...
            if days < 0:
                error_msg = f"Days ago must be positive, got {days}"
                raise ValidationError(error_msg)
            return date_str.lower()
...
def validate_max_commits(value: Any) -> int:
    ...
    try:
        ...
        error_msg = f"max_commits must be an integer, got {type(value).__name__}"
        raise ValidationError(error_msg) from err

    if value < 1:
        error_msg = f"max_commits must be at least 1, got {value}"
        raise ValidationError(error_msg)

    if value > MAX_COMMITS_PER_DAY:
        error_msg = f"max_commits must be at most {MAX_COMMITS_PER_DAY}, got {value}"
        raise ValidationError(error_msg)
...
def validate_frequency(value: Any) -> int:
    ...
    try:
        ...
        error_msg = f"frequency must be an integer, got {type(value).__name__}"
        raise ValidationError(error_msg) from err

    if value < 0:
        error_msg = f"frequency must be at least 0, got {value}"
        raise ValidationError(error_msg)

    if value > MAX_FREQUENCY:
        error_msg = f"frequency must be at most {MAX_FREQUENCY}, got {value}"
        raise ValidationError(error_msg)
...
def validate_country_code(country: str) -> str:
    ...
    if not isinstance(country, str):
        error_msg = f"country_holidays must be a string, got {type(country).__name__}"
        raise ValidationError(error_msg)
    ...
    if country not in holidays.__dict__:
        ...
        supported = sorted(
            [
                ...
                if not name.startswith("_") and len(name) == MIN_URL_PARTS - 1
            ]
        )
...
def validate_repository_url(url: str) -> str:
    ...
    if not isinstance(url, str):
        error_msg = f"repository_url must be a string, got {type(url).__name__}"
        raise ValidationError(error_msg)

    url = url.strip()

    if not url:
        error_msg = "repository_url cannot be empty"
        raise ValidationError(error_msg)
...
def validate_git_user_config(
    ...
    if name is not None:
        if not isinstance(name, str):
            error_msg = f"user_name must be a string, got {type(name).__name__}"
            raise ValidationError(error_msg)
        name = name.strip()
        if not name:
            name = None
    ...
    if email is not None:
        if not isinstance(email, str):
            error_msg = f"user_email must be a string, got {type(email).__name__}"
            raise ValidationError(error_msg)
        email = email.strip()
        if not email:
            email = None
...
def validate_boolean(value: Any, field_name: str) -> bool:
    ...
    if not isinstance(value, bool):
        error_msg = f"{field_name} must be a boolean, got {type(value).__name__}"
        raise ValidationError(error_msg)
...
def validate_directory_path(path: str) -> str:
    ...
    if not isinstance(path, str):
        error_msg = f"Directory path must be a string, got {type(path).__name__}"
        raise ValidationError(error_msg)
    ...
    for char in invalid_chars:
        if char in path:
            error_msg = f"Directory path contains invalid character: {char}"
            raise ValidationError(error_msg)
...
def validate_file_path(path: str) -> str:
    ...
    """
    if not isinstance(path, str):
        error_msg = f"File path must be a string, got {type(path).__name__}"
        raise ValidationError(error_msg)
    ...
    for char in invalid_chars:
        if char in path:
            error_msg = f"File path contains invalid character: {char}"
            raise ValidationError(error_msg)
...
def validate_positive_integer(
    value: Any, field_name: str, max_value: Optional[int] = None
) -> int:
    ...
    try:
        ...
        error_msg = f"{field_name} must be an integer, got {type(value).__name__}"
        raise ValidationError(error_msg) from err

    if value <= 0:
        error_msg = f"{field_name} must be positive, got {value}"
        raise ValidationError(error_msg)

    if max_value is not None and value > max_value:
        error_msg = f"{field_name} must be at most {max_value}, got {value}"
        raise ValidationError(error_msg)
