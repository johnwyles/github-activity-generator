...
class ActivityGenerator:
    ...
    def __init__(self, config: Config):
        ...
        ) if config.output.dry_run else None
        ...
        if config.commit_behavior.skip_holidays:
            try:
                holiday_class = getattr(holidays, config.commit_behavior.holiday_country)
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            if self.config.git_settings.repository_url and not self.config.output.dry_run:
    ...
    def _validate_date_range(self, start_date: datetime, end_date: datetime) -> None:
        ...
        if start_date > current_date:
            logger.warning(
                f"{Colors.BLUE}Start date ({format_date(start_date)}) is in the future.{Colors.RESET}"
            )
        ...
        if end_date > current_date:
            logger.warning(
                f"{Colors.BLUE}End date ({format_date(end_date)}) is in the future.{Colors.RESET}"
            )
            if not self._confirm_action("Continue anyway?"):
                sys.exit(0)
        ...
        logger.info(
            f"Will generate commits from {Colors.CYAN}{format_date(start_date)}{Colors.RESET} "
    ...
    def _setup_repository(self) -> Path:
        ...
        if self.config.git_settings.repository_url:
            dir_name = extract_repo_name_from_url(self.config.git_settings.repository_url)
            if not dir_name:
                dir_name = f"repository-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
        else:
            dir_name = f"repository-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
    ...
    def _should_commit_on_date(self, date: datetime) -> Tuple[bool, str]:
        ...
        """
        # Check weekend
        if self.config.commit_behavior.skip_weekends and date.weekday() >= 5:
            return False, "weekend"
        
        # Check holiday
        if self.config.commit_behavior.skip_holidays and self.holidays:
    ...
    def _show_completion_message(
        ...
        logger.info(
            f"\n{Colors.GREEN}Repository generation completed successfully!{Colors.RESET}"
        )
        logger.info(
            f"Created {Colors.BOLD}{total_commits}{Colors.RESET} {pluralize(total_commits, 'commit')} "
            f"over {Colors.BOLD}{total_days}{Colors.RESET} {pluralize(total_days, 'day')}"
        )